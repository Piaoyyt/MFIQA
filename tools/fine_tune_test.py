...
#from Evaluation.FIQA_model.FIQA import FIQA_mine
...
def build_test_data():
    ...
    test_shuffle = shuffle(test_paths[-200:])
...
def model_predict(predict_model,database, batch, batch_number):
    ...
    for i in range(batch_number):
        ...
        with torch.no_grad():
            ...
            output = np.squeeze(predict_model(test_batch.to(device), test_batch_mask.to(device)).detach().cpu().numpy())#,test_batch_mask
...
def main():
    ...
    predict_model = torch.load(model_path).to(device).eval()
    predict_model.device_ids = [0]
    #predict_model = FIQA_mine().model#
    ...
    test_images_num = 10
    row = 2
    # data = test_database.__getitem__(test_index[10])
    # print(test_database.img_paths[test_index[10]])
    # img = Image.open(test_database.img_paths[test_index[10]])
    # plt.imshow(img)
    # plt.pause(3)
    # plt.ioff()
    # plt.clf()
    ...
    # score = predict_model(data["input"].view(-1,3,112,112).cuda())
    # print(score)
    ...
    for i in range(row):
        for j in range(colums):
            ...
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)#data["input_mask"].view(-1,3,160,160).cuda()
            predict = round(predict_model(data["input"].view(-1,3,160,160).cuda(),\
                                          data["input_mask"].view(-1,3,160,160).cuda()).item(),\
                                         3)
            try:
                ground_truth = round(data["quality_label"].item(),2)
            except:
                ground_truth = round(data["quality_label"], 2)
            # print(groud_truth)
    ...
    correlation_f = "/".join(model_path.split("/")[:-1])+"/"+model_path.split("/")[-1].split(".")[0]+"_correlation.txt"
    #pic_path = "./FIQA_model_result.jpg"
    ...
    # # plt.legend(loc="lower right")
    # # plt.show()
    correlation_analysis(test_label,predict_output,correlation_f)
